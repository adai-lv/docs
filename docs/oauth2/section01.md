...
- 客户端
  ...
  使用资源所有者的授权，代表资源所有者发起对受保护资源的请求的应用程序。术语“客户端”并非特指任何特定的的实现（例如：应用程序是否是在服务器、台式机或其他设备上执行）。
...
客户端从资源所有者获得授权许可（步骤（A）和（B）所示）的更好方法是使用授权服务器作为中介，如[授权码许可][授权码许可]图3所示。
...
当在隐式许可流程中颁发访问令牌时，授权服务器不对客户端进行身份验证。在某些情况下，客户端身份可以通过用于向客户端传送访问令牌的重定向URI验证。访问令牌可能会暴露给资源所有者，或者对资源所有者的用户代理有访问权限的其他应用程序。

隐式许可提高了一些客户端（例如一个作为浏览器内应用实现的客户端）的响应速度和效率，因为它减少了获取访问令牌所需的往返次数。然而，这种便利应该和采用隐式许可的安全影响作权衡，如那些在[安全考量-访问令牌][安全考量-访问令牌]和[安全考量-在隐式流程中滥用访问令牌假冒资源所有者][安全考量-在隐式流程中滥用访问令牌假冒资源所有者]节中所述的，尤其是当授权码许可类型可用的时候。
...
当授权范围限于客户端控制下的受保护资源或事先与授权服务器商定的受保护资源时，客户端凭据可以被用作为一种授权许可。典型的当客户端代表自己表演（客户端也是资源所有者）或者基于与授权服务器事先商定的授权请求对受保护资源的访问权限时，客户端凭据被用作为授权许可。
...
步骤（C）、（D）、（E）和（F）在本规范的范围以外，如[访问受保护资源][访问受保护资源]中所述。
...
[授权码许可]: oauth2/section04#授权码许可 "授权码许可"
[安全考量-访问令牌]: oauth2/section10#访问令牌 "安全考量-访问令牌"
[安全考量-在隐式流程中滥用访问令牌假冒资源所有者]: oauth2/section10#在隐式流程中滥用访问令牌假冒资源所有者 "安全考量-在隐式流程中滥用访问令牌假冒资源所有者"
[RFC6750]: http://tools.ietf.org/html/rfc6750 "OAuth 2.0授权框架：承载令牌的使用"
[访问受保护资源]: oauth2/section06#访问受保护资源 "访问受保护资源"
